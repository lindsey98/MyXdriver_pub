...
function install_chrome {
	rpmfile="google-chrome-stable_current_x86_64.rpm"
	wget "https://dl.google.com/linux/direct/$rpmfile" -P "$install_dir"
	if [ $? -ne 0 ];
	then
		fail "Could not download Chrome"
		return 1
	fi
	sudo yum localinstall -y "$install_dir$rpmfile"
	if [ $? -ne 0 ];
	then
...
enabled=1" | sudo tee -a /etc/yum.repos.d/opera.repo

	if [ $? -ne 0 ];
	then
		fail "Could not add Opera yum repository"
		return 2
	fi

	sudo yum install -y opera-stable
	if [ $? -ne 0 ];
	then
